...
namespace kantan
{
        ...
        for(kantan::Component* component : aabbHitboxComponents)
        {
            ...
            for(kantan::Component* secondComponent : aabbHitboxComponents)
            {
                ...
                if(fstNewHitbox.intersects(sndHitbox->hitbox))
                {
                    ...
                    m_collisions.push_back(std::pair<unsigned long long, unsigned long long>(fstHitbox->getOwnerId(), sndHitbox->getOwnerId()));
    ...
    std::vector<std::pair<unsigned long long, unsigned long long>> AABBCollisionSystem::getCollisionRecord()
    {
        return m_collisions;
    }
} // namespace kantan.
